...
作者的素材大多先落在笔记里，而这本书的设定又需要在写的时候随时能翻。
所以两边各走一条路，都在「碎片」面板里操作：

**素材进系统**：在库里建一个收件箱目录（默认 `素材`），笔记扔进去就变成想法碎片，
`frontmatter` 可以指定类型、意图、标签、相关人物、目标章号：
...
雨没停。他蹲在石阶上擦刀——台阶尽头的雾里，有人一直在看他。 #关键场景
```

导入是幂等的：同一篇笔记反复导入只会更新正文，不会变成一堆重复碎片。
收件箱以外的笔记不会被吸进来，除非带 `novelos` 标记。

**设定出到系统外**：一键把人物、事件、伏笔、Canon、世界观、大纲导出成带 `[[双链]]` 的笔记，
放在库里的 `NovelOS/<书名>/` 下，可以在 Obsidian 里用图谱视图看自己故事的关系网。
你在库里改过的那些笔记**不会被覆盖** —— 重导出时新内容另存为 `.conflict.md`，由你自己合并。

库路径不用手填：默认从 Obsidian 自己的配置里探测（通常是当前打开的那个），
也可以用 `NOVELOS_OBSIDIAN_VAULT` 指定，或在界面上改。
